...
def _call_llm(prompt: str) -> str:
    """Call the brainstem's Copilot API to generate code. Returns raw text or empty string."""
    try:
        from brainstem import call_copilot
        messages = [
            {"role": "system", "content": "You are a Python code generator. Return ONLY valid Python code. No markdown fences, no explanations."},
            {"role": "user", "content": prompt}
        ]
        resp = call_copilot(messages)
        ...
        print(f"[LearnNew] LLM call failed, using fallback: {e}")
        return ""
...
'''
    ...
    def _generate_name(self, description: str) -> str:
        ...
        # Try LLM first
        name = _call_llm(f"Generate a short 1-2 word CamelCase name for an agent that: {description[:200]}. Reply with ONLY the name, nothing else.")
        if name:
            name = re.sub(r'[^a-zA-Z]', '', name.split('\n')[0])
    ...
    def _generate_perform_body(self, description: str) -> str:
        """Generate the perform method body using the brainstem's Copilot API."""
        prompt = f"""Write the body of a Python method for an agent that: {description}

Context: This code goes inside `def perform(self, **kwargs):` which is inside a class.
The variable `query = kwargs.get('query', '')` is already defined.
The module `json` is already imported at the top of the file.

Return ONLY the method body lines. Use 0 indentation for the top level (I will re-indent).
Do NOT include `def perform` or any class/method signature.
...
try:
    ...
    return json.dumps({{"status": "error", "message": str(e)}})"""
        ...
        if body:
            ...
            # Find the minimum indentation to rebase to 0
            indents = [len(l) - len(l.lstrip()) for l in lines if l.strip()]
            min_indent = min(indents) if indents else 0
            # Rebase to 8-space indent (method body inside a class)
            rebased = []
            for line in lines:
                if line.strip():
                    rebased.append('        ' + line[min_indent:])
                else:
                    rebased.append('')
            indented = '\n'.join(rebased)
            if indented.strip():
